...
# Normalizált név a kereséshez
df["nev_clean"] = df["Területi egység neve"].astype(str).str.replace(r"\s+", "", regex=True)

# Összesen blokk határai
start = df.index[df["nev_clean"] == "Összesen"][0] + 1
end   = df.index[df["nev_clean"] == "Országösszesen"][0] - 1
